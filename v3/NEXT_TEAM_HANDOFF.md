...
- The stuck-photo / missing-photo-minimum regression was fixed.

### Dispatch and property model
...
- The current repo also hardens property create/edit payloads so blank optional fields are omitted instead of sent as explicit `undefined`.
...
- This mattered directly during the Client / Account rollout on properties.
- Backend deploy script lives in `packages/backend/package.json`:
  - `bun run deploy`

## Capacity / Usage Notes
- The latest Convex production deploy warned that the project is above Free plan limits.
...
- Real-device mobile QA is still essential for photo capture and native chooser behavior.
...
- Convex bandwidth/storage/plan headroom now needs active monitoring.
- The app has real production data now, so any schema/auth/env change should be treated as a rollout change.
...
5. Check Convex usage/plan impact before large photo or query changes.
